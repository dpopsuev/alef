...
Examples:
  ...
  npx @dpopsuev/alef-ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @dpopsuev/alef-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @dpopsuev/alef-ai --help' for usage`);
	process.exit(1);
}
