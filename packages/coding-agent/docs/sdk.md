...
```bash
npm install @dpopsuev/alef-coding-agent
```
...
The `Agent` class (from `@dpopsuev/alef-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "<agent-dir>", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.alef/extensions/`)
- Project skills:
  - `.alef/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.alef/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `<agent-dir>/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@dpopsuev/alef-coding-agent";
...
// Default: uses <agent-dir>/auth.json and <agent-dir>/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@dpopsuev/alef-coding-agent";
...
import { createAgentSession, defineTool } from "@dpopsuev/alef-coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `<agent-dir>/extensions/`, `.alef/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@dpopsuev/alef-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@dpopsuev/alef-coding-agent";
...
1. Global: `<agent-dir>/settings.json`
2. Project: `<cwd>/.alef/settings.json`
