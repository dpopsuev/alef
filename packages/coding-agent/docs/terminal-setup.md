...
If you want `Shift+Enter` to keep working in tmux via that remap, add `ctrl+j` to your pi `newLine` keybinding in `<agent-dir>/keybindings.json`:
...
If you want the hardware cursor visible, set `ALF_HARDWARE_CURSOR=1` before running pi (disabled by default for compatibility).
