...
import type { Component, EditorTheme, TUI } from "@alf-agent/tui";
...
export default function (alf: ExtensionAPI) {
	...
	alf.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await alf.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = alf.getThinkingLevel();
