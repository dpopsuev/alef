 ...
 *
 * Usage:
 *   alef -e ./ssh.ts --ssh user@host
 *   alef -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (alef: ExtensionAPI) {
	alef.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	alef.on("session_start", async (_event, ctx) => {
		...
		const arg = alef.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
