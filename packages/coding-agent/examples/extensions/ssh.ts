 ...
 *
 * Usage:
 *   alf -e ./ssh.ts --ssh user@host
 *   alf -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
export default function (alf: ExtensionAPI) {
	alf.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	alf.on("session_start", async (_event, ctx) => {
		...
		const arg = alf.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
