	...
	relocateTopic(request: RelocateDiscourseTopicRequest): BlackboardTopicSummary {
		...
		const updatedTopic: DiscourseTopic = {
			...
			labels: this.entityLabels(board, forum, {
				...
				extra: [
					{ key: "relocated-by", value: request.relocatedBy, source: "coordinator" },
					...(request.reason ? [{ key: "reason", value: request.reason, source: "coordinator" as const }] : []),
					...(request.labels ?? []),
				],
				fallbackSource: "coordinator",
