 ...
 *
 */
export interface PlanShelfRecord {
	phase?: string;
	desired?: string;
	current?: string;
...
/**
 * Shared focused-plan → preview DTO. Used by loadPlanPreview and PlanStore.
 */
export function planPreviewFromData(data: PlanShelfRecord): PlanPreviewInput | undefined {
	...
	if (typeof data.summary === "string" && data.summary) {
		parts.push(data.summary.slice(0, PLAN_SUMMARY_PREVIEW_CHARS));
	}
	return {
		phase: data.phase,
...
export async function loadPlanPreview(cwd: string | undefined): Promise<PlanPreviewInput | undefined> {
	...
	try {
		...
		// eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- shelf JSON from PlanGraph.toJSON
		const data = JSON.parse(raw) as PlanShelfRecord;
		return planPreviewFromData(data);
	} catch {
		return undefined;
	}
}
