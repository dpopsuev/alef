...
export function applyTextEdit(content: string, oldText: string, newText: string, path: string): string {
	...
	if (first === -1) throw new Error(`code.edit: oldText not found in ${path}`);
	const last = content.lastIndexOf(oldText);
	if (first !== last) throw new Error(`code.edit: oldText matches multiple locations in ${path} — make it unique`);
