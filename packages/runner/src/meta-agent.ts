...
const META_SYSTEM_PROMPT =
	...
	"Use alef.scroll.list to show the prompt scroll blocks. Use alef.scroll.enable/disable/toggle/replace to modify them. " +
...
export async function runMetaAgent(
	...
	const strategy = new InProcessStrategy([createAlefApiOrgan({ getScroll })], model, META_SYSTEM_PROMPT);
