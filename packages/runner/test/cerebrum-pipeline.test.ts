/**
 * Cerebrum pipeline integration tests.
 *
 * Exercises the parts ScriptedReasoner cannot reach:
 *   - prepareStep called by real Cerebrum; output visible in the LLM context
...
/** Minimal agent: Cerebrum with faux LLM + DialogOrgan. */
function makeAgent(
...
describe("motor/llm.phase pipeline", () => {
	...
	it("ToolShell phaseStage adds tools.describe to ctx.tools on turn 1", async () => {
		...
		// Cerebrum substitutes dots with underscores for the LLM wire format.
