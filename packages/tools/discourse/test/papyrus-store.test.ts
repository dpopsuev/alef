...
describe("Papyrus Discourse adapter", () => {
	it("maps every persistence operation through one authenticated namespaced authority", async () => {
		...
		const address = { boardId: "acme", forumId: "engineering", topicId: "reviews", threadId: "mesh" };
		const command = {
		...
		await store.listTopics({ boardId: address.boardId, forumId: address.forumId, limit: 10 });
		await store.listThreads({
			boardId: address.boardId,
			forumId: address.forumId,
