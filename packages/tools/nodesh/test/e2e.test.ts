...
describe.skipIf(!HAVE_REAL_LLM)("nodesh — real LLM E2E", { tags: ["real-llm"] }, () => {
	it("LLM evaluates a JS expression using nodesh.eval and reports the result", async () => {
		const session = await createE2eSession([createNodeshAdapter({ cwd: process.cwd() })]);
		try {
			...
			expect(
				events.some((e) => e.type === "llm.tool-start" && String(e.payload.name ?? "").includes("nodesh")),
			).toBe(true);
		} finally {
