 ...
 *
 * Creates packages/organ-<name>/ with:
...
const pkgDir = join(process.cwd(), "packages", `organ-${name}`);
...
export default mergeConfig(sharedConfig, defineProject({ test: { name: "organ-${name}" } }));
`,
);

console.log(`Created packages/organ-${name}/`);
...
console.log(`  3. cd packages/organ-${name} && npx vitest run`);
